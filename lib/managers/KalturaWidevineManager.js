...
class KalturaWidevineManager extends drmManager.KalturaDrmManager
{
    ...
    start(app)
    {
        app.post('/cenc/encryption', (request, response) => this.encryption(request,response));
        app.post('/cenc/widevine/license', (request, response) => this.license);
    }
    ...
    {
        let This = this;        
        let dbKey = this._drmType + '_' + postParams.account_id + '_' + postParams.content_id + '_' + postParams.files;
        let encryptionDataPromise = kalturaCouchbaseConnector.getInstance().get(dbKey, true);
    ...
    getLicenseResponse(providerData, customData, payload, entResponse, files)
    {
        ...
        let contentKeys = this._getFiles(customData.files, customData.content_id, files, dbPrefix);
    ...
    {
        if (typeof verifiedFiles == 'undefined')
