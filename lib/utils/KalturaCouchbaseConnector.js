...
var KalturaCouchbaseConnector = {
    ...
    init : function() {
        KalturaLogger.log("@@NA cluster name ["+this.clusterName+"] bucket name [" + this.bucketName + "]");
    ...
    get : function(key, callback)
    {
        this.bucket.get('key', function (err, result)
        {
            if (err)
            {
                KalturaLogger.log("Error in getting key msg ["+JSON.stringify(err)+"]");
            }
            else {
                KalturaLogger.log("Got result [" + result + "]");
            }
            callback(err, result);
        });
    },
    ...
    {
        //KalturaLogger.log("@@NA cluster name ["+this.clusterName+"] bucket name [" + this.bucketName + "]");
        //this.cluster = new couchbase.Cluster('chouchbase://' + this.clusterName);//TODO: take this parameter from config
        //this.bucket = this.cluster.openBucket("uDRM");//TODO: take this parameter from config
