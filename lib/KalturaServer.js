...
KalturaChildProcess.prototype.configureExpressApp = function (app)
{
    ...
    app.get('/clientaccesspolicy.xml',
        function(request, response)
        {
           response.send('<access-policy><cross-domain-access><policy><allow-from http-request-headers="*"><domain uri="http://*"/><domain uri="https://*"/></allow-from><grant-to><resource path="/" include-subpaths="true"/></grant-to></policy></cross-domain-access></access-policy>');
        });
	...
	{
        let data = '';
        KalturaLogger.debug(`@@NA content type given [${util.inspect(req.headers['content-type'])}]`);
